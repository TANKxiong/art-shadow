...
const CHANGELOG = [
    ...
    version: '1.4.5',
    date: '2026-08-05',
    title: '预览镜像翻转 + Maya 角色工具',
    items: [
      '预览窗口新增「🪞 水平镜像」：视频/图片一键水平翻转（纯显示，不影响原始素材），做对称动作参考更方便',
      'Maya 插件新增「角色工具」分区：',
      '  - 定在原地：自由选择冻结位移轴(X/Y/Z)和旋转轴(X/Y/Z)，检查循环动画用',
      '  - 原地循环走：一键把向前走的动画转为原地循环走（身体原地踏步，脚不滑步不乱飞）',
      'Maya 插件支持「拖入即用 + 自动安装」：拖入视图即弹窗口，并自动复制到 scripts 目录，重启菜单永久生效',
      'Maya 插件打包为便携安装包，可跨电脑使用（maya_plugin 目录）'
    ]
  },
{
    version: '1.4.4',
  ...
  },
{
    ...
    title: 'Maya 参考素材导入优化',
    items: [
      '序列帧导出改为 Maya 点号格式（ref.1.png），Maya 插件直接识别，不再生成第二套序列',
      'Maya 插件界面重做：导入工具 / 多视角参考 / 时间轴工具 / 日志 分区归类，可折叠面板',
      'Maya 菜单归类：「画影客」菜单下分 导入工具 / 时间轴工具 / 帮助 子菜单',
      '修复 imagePlane 跟随相机：平面挂载为相机子节点，相机移动时参考面跟随',
      '修复序列播放：点号格式 + 单 # 占位，时间轴逐帧播放；默认不循环播完停在末帧',
      '修复 _maya_seq 目录嵌套：重复导入不再套娃生成重复序列',
      '超大图片提示：序列帧超过 2048px 时提醒可能卡顿，建议导出小分辨率'
    ]
  },
{
    version: '1.4.2',
